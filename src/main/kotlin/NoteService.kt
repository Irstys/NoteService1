class NoteService {
    ...
    fun add(title: String, text: String, ownerId: Int): Int {
        noteId += 1
        notes.add(Note(title = title, text = text, noteId = noteId, ownerId =ownerId))
        return noteId
    }
    ...
    fun createComment(noteId: Int, message: String, userId: Int, ownerId: Int): Int {
        for (note in notes) {
            if (note.noteId == noteId) {
                commentId += 1
                note.comments.add(Comment(message = message, commentId = commentId, noteId = noteId, userId=userId, ownerId =ownerId))
                return commentId
            }
        }
        return 0
    ...
    fun getById(noteId: Int): Note {
        ...
        throw NoteNotFoundException("Заметки с id $noteId нет")
    }
